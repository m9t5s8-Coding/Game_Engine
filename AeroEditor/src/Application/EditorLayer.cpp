...
void EditorLayer::render_transform_tools_group()
{
    ...
    // Move button
    render_transform_button(ICON_FA_ARROWS_UP_DOWN_LEFT_RIGHT, "Grab ( G )", TransformSetting::Move,
                            !has_selection);
    ImGui::SameLine(0, 8);

    // Rotate button
    render_transform_button(ICON_FA_ROTATE, "Rotate ( R )", TransformSetting::Rotate,
                            !has_selection);
    ImGui::SameLine(0, 8);
...
void EditorLayer::render_transform_button(const char* label, const char* shortcut,
                                          TransformSetting mode, bool disabled)
{
    auto current_transform_mode = m_panel->get_transform_setting();
...
void EditorLayer::render_axis_button(const char* label, const char* shortcut, TransformAxis axis,
                                     bool disabled)
{
...
void EditorLayer::render_paint_button(const char* label, const char* shortcut,
...
void EditorLayer::update_mouse_position()
{
    ...
    if (m_view_controller)
    {
        ...
        m_current_mouse_pos = Math::screen_to_world(
            m_current_mouse_pos, m_view_controller->get_view().get_float_rect(), m_viewport_size);
    }
...
void EditorLayer::editor_things()
{
    ...
    // x axis and y axis
    {
        ...
        x_axis_transform.position = Math::world_to_screen(x_axis_transform.position,
                                                          view.get_float_rect(), m_viewport_size);
        y_axis_transform.position = Math::world_to_screen(y_axis_transform.position,
                                                          view.get_float_rect(), m_viewport_size);
    ...
    // Camera
    {
        ...
        for (auto& entityID : entities)
        {
            ...
            if (entity.has_component<Camera_Component>())
            {
                ...
                top_right =
                    Math::world_to_screen(top_right, view.get_float_rect(), m_viewport_size);
                bottom_left =
                    Math::world_to_screen(bottom_left, view.get_float_rect(), m_viewport_size);
                bottom_right =
                    Math::world_to_screen(bottom_right, view.get_float_rect(), m_viewport_size);
...
void EditorLayer::draw_transform_settings(Entity e)
{
    ...
    x_axis_transform.position =
        Math::world_to_screen(x_pos, view.get_float_rect(), m_viewport_size);
    y_axis_transform.position =
        Math::world_to_screen(y_pos, view.get_float_rect(), m_viewport_size);
