...
void Texture_PopUp::draw_popup(PopUpModel& model, Extra_Settings& extra_settings, Entity entity)
{
  ...
  {
    model.window_size = {Application::get().get_window().get_size().x * 0.7f,
                         Application::get().get_window().get_size().y * 0.8f};
  }
  ...
  ImGui::SetNextWindowPos(
      ImVec2(center.x - model.window_size.x * 0.5f, center.y - model.window_size.y * 0.5f),
      ImGuiCond_Appearing);
...
void Texture_PopUp::draw_content(PopUpModel&     model,
  ...
  ImGui::BeginChild("TextureColumn", ImVec2(left_width, 0), true);
  {
                      ...
                      ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);
    {
      ...
      if (settings.is_dragging)
      {
        ...
        {
          vec2f start =
              Math::screen_to_world(settings.last_mouse_pos,
                                    Math::get_float_rect(settings.view_size, settings.view_center),
                                    container);
          vec2f end =
              Math::screen_to_world(settings.current_mouse_pos,
                                    Math::get_float_rect(settings.view_size, settings.view_center),
                                    container);
      ...
      vec2f screen_pos =
          Math::world_to_screen(image_pos,
                                Math::get_float_rect(settings.view_size, settings.view_center),
                                container);
      vec2f screen_size =
          Math::world_size_to_screen_size(texture_size, settings.view_size, container);
...

      {
        ...
        vec2f button_size =
            Math::world_size_to_screen_size(tile_size, settings.view_size, container);
        vec2f scale = screen_size / texture_size;
        if (extra_settings.create_buttons)
          create_buttons(screen_pos,
                         screen_size,
                         tile_size,
                         line_count,
                         texture_size,
...
void Texture_PopUp::draw_lines(const vec2f& screen_pos,
...
void Texture_PopUp::create_buttons(const vec2f&    screen_pos,
                                   const vec2f&    screen_size,
                                   const vec2i&    size,
                                   const vec2i&    line_count,
                                   const vec2i&    texture_size,
