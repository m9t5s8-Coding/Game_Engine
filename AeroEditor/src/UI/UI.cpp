...
void UI::draw_texture(Entity entity)
{
                    ...
                    ImGuiWindowFlags_NoScrollbar);
  {
    ...
    draw_list->AddRect(frame_min, frame_max, ImGui::GetColorU32(ImGuiCol_Border), 4.0f, 0, 1.5f);
...
void UI::draw_folder_node(const std::filesystem::path& directory, int depth)
{
  ...
  for (const auto& entry : entries)
  {
    ...
    if (entry.is_directory())
    {
      ...
      bool is_open = ImGui::TreeNodeEx((std::string(ICON_FA_FOLDER) + "  " + name).c_str(), flags);
    ...
    else if (is_right_file(path))
    {
      ImGuiTreeNodeFlags flags =
          ImGuiTreeNodeFlags_Framed | ImGuiTreeNodeFlags_Leaf | ImGuiTreeNodeFlags_SpanAvailWidth |
...
void UI::draw_animation(Entity entity)
{
  ...
  ImGui::InputTextWithHint("##NewAnimName",
                           "New Animation Name",
                           new_anim_name,
...
void UI::draw_frame_selector(Entity entity, const std::string& current_animation)
{
  ...
  extra_settings.button_controls = [&](const Button_Control_Value& details)
  {
    ...
    vec2f cell_min = details.screen_pos +
                     vec2f(details.tile_id.x * details.size.x, details.tile_id.y * details.size.y) *
                         details.scale;
    ...
    if (is_selected)
    {
      details.draw_list->AddRectFilled(cell_min.to_imvec2(),
                                       cell_max.to_imvec2(),
                                       IM_COL32(0, 255, 0, 80));
      details.draw_list->AddRect(cell_min.to_imvec2(),
                                 cell_max.to_imvec2(),
  ...
  extra_settings.controls_panel = [&]()
  {
    ...
    ImGui::Text("Selected: %d/%d frames", (int)selected_frames.size(), total_grid.x * total_grid.y);
          ...
          ImGui::BeginChild("TextureColumn", ImVec2(left_width, 0), true);
          {
                              ...
                              ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);
            {
              ...
              if (is_dragging)
              {
                ...
                {
                  vec2f start = Math::screen_to_world(last_mouse_pos,
                                                      Math::get_float_rect(view_size, view_center),
                                                      container);
                  vec2f end   = Math::screen_to_world(current_mouse_pos,
                                                    Math::get_float_rect(view_size, view_center),
                                                    container);
              ...
              if (is_hovering_container && ImGui::GetIO().MouseWheel != 0)
              {
                ...
                vec2f world_before_zoom =
                    Math::screen_to_world(current_mouse_pos,
                                          Math::get_float_rect(view_size, view_center),
                                          container);
                view_size *= scale_factor;
                vec2f world_after_zoom =
                    Math::screen_to_world(current_mouse_pos,
                                          Math::get_float_rect(view_size, view_center),
                                          container);
              ...
              vec2f screen_pos = Math::world_to_screen(world_pos,
                                                       Math::get_float_rect(view_size, view_center),
                                                       container);
              vec2f screen_size =
              ...
              for (int x = 0; x < total_dim.x; x++)
              {
                for (int y = 0; y < total_dim.y; y++)
                {
                  ...
                  bool is_selected =
                      std::find(selected_tiles.begin(), selected_tiles.end(), tile_id) !=
                      selected_tiles.end();

                  bool is_solid = std::find(solid_tiles.begin(), solid_tiles.end(), tile_id) !=
                                      solid_tiles.end() &&
                                  is_selected;
                  ...
                  vec2f cell_max =
                      screen_pos + vec2f((x + 1) * tile_size.x, (y + 1) * tile_size.y) * scale;

                  ImU32 selected_color =
                      is_selected ? IM_COL32(200, 200, 200, 50) : IM_COL32(0, 0, 0, 150);
                  ...
                  if (ImGui::InvisibleButton("##Cell", button_size.to_imvec2()))
                  {
                    ...
                    if (control)
                    {
                      if (!is_for_solid)
                      {
                        auto it = std::find(selected_tiles.begin(), selected_tiles.end(), tile_id);
                      ...
                      else
                      {
                        ...
                        {
                          auto it = std::find(solid_tiles.begin(), solid_tiles.end(), tile_id);
                      ...
                      if (!is_for_solid)
                      {
                        ...
                        for (int y = start_y; y <= end_y; y++)
                        {
                          for (int x = start_x; x <= end_x; x++)
                          {
                            ...
                            if (std::find(selected_tiles.begin(), selected_tiles.end(), id) ==
                                selected_tiles.end())
                            {
                              selected_tiles.push_back(id);
                            }
                        ...
                        for (int y = start_y; y <= end_y; y++)
                        {
                          for (int x = start_x; x <= end_x; x++)
                          {
                            ...
                            if (std::find(selected_tiles.begin(), selected_tiles.end(), id) !=
                                selected_tiles.end())
                            {
                              if (std::find(solid_tiles.begin(), solid_tiles.end(), id) ==
                                  solid_tiles.end())
                              {
                                solid_tiles.push_back(id);
                              }
                    ...
                    else
                    {
                      ...
                      {
                        if (std::find(selected_tiles.begin(), selected_tiles.end(), tile_id) !=
                            selected_tiles.end())
                        {
                          solid_tiles.clear();
                          solid_tiles.push_back(tile_id);
          ...
          ImGui::BeginChild("Control", ImVec2(right_width, 0), true);
          {
                ...
                !selected_tiles.empty())
            {
              ...
              for (const auto& ids : selected_tiles)
              {
                ...
                def.is_solid =
                    std::find(solid_tiles.begin(), solid_tiles.end(), ids) != solid_tiles.end();
                ...
                for (int y = 0; y < total_dim.y; y++)
                {
                  ...
                  for (int sy = 0; sy < 3; sy++)
                  {
                    for (int sx = 0; sx < 3; sx++)
                    {
                      ...
                      ImU32 color = selected ? IM_COL32(255, 0, 0, 180) : IM_COL32(50, 50, 50, 0);
  ...
  if (ImGui::Begin("TileSet Selector", &window_open, flags))
  {
    ...
    ImGui::BeginChild("TextureColumn", ImVec2(0, 0), true);
    {
      if (entity.has_component<AutoTiling_Component>())
      {
        ...
        {
          if (ImGui::BeginCombo("Current Set", set_name.empty() ? "None" : set_name.c_str()))
          {
        ...
        {  // On Resize
          ...
          for (int x = 0; x < total_dim.x; x++)
          {
            for (int y = 0; y < total_dim.y; y++)
            {
              ...
              bool is_registered =
                  std::find(registered_tiles.begin(), registered_tiles.end(), tile_id) !=
  ...
  if (ImGui::Begin("Texture Selector", &window_open, flags))
  {
    ...
    ImGui::BeginChild("TextureColumn", ImVec2(0, 0), true);
    {
                        ...
                        ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);
      {
        ...
        vec2f screen_pos  = Math::world_to_screen(image_pos,
                                                 Math::get_float_rect(view_size, view_center),
                                                 container);
        vec2f screen_size = Math::world_size_to_screen_size(texture_size, view_size, container);
        ImGui::SetCursorPos(screen_pos.to_imvec2());
        ...
        if (is_hovered && ImGui::IsMouseClicked(0))
        {
          ...
          vec2f world_pos    = Math::screen_to_world(current_mouse_pos,
                                                  Math::get_float_rect(view_size, view_center),
                                                  container);
        ...
        if (is_hovered && ImGui::IsMouseDown(0))
        {
          ...
          vec2f start = Math::world_to_screen(start_pos + image_pos,
                                              Math::get_float_rect(view_size, view_center),
                                              container);
...
void UI::draw_loaded_script_panel(Entity entity, Script_Component& props)
{
  ...
  if (ImGui::BeginPopupContextItem("##script_context_menu", ImGuiPopupFlags_MouseButtonRight))
  {
    ...
    if (ImGui::MenuItem(ICON_FA_LINK "  Open Containing Folder",
                        nullptr,
                        false,
                        !props.path.empty()))
    {
...
void UI::save_changes()
{
  ...
  model.draw_content = [&]()
  {
    ...
    ImGui::SetCursorPosX(ImGui::GetCursorPosX() + spacing);
    ImGui::SetCursorPosY(ImGui::GetCursorPosY() + ImGui::GetContentRegionAvail().y - button.size.y -
                         spacing);
