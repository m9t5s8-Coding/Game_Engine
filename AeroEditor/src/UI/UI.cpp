...
namespace ag
{
static const uint16_t bit_lookup[3][3] = {{TL, T, TR}, {L, M, R}, {BL, B, BR}};
...
void UI::draw_texture(Entity entity)
{
    ...
    ImVec2      frame_size = ImVec2(preview_size.x + padding * 2, preview_size.y + padding * 2);

    float available_width = ImGui::GetContentRegionAvail().x;
    float frame_pos_x     = (available_width - frame_size.x) / 2.0f;

    ImGui::BeginChild("TexturePreviewFrame", ImVec2(0, frame_size.y + 5), false,
                      ImGuiWindowFlags_NoScrollbar);
    {
        ...
        ImVec2      frame_max = ImVec2(frame_min.x + frame_size.x, frame_min.y + frame_size.y);

        draw_list->AddRectFilled(frame_min, frame_max, ImGui::GetColorU32(ImGuiCol_FrameBg), 4.0f);

        draw_list->AddRect(frame_min, frame_max, ImGui::GetColorU32(ImGuiCol_Border), 4.0f, 0,
                           1.5f);
        ...
        if (ImGui::BeginDragDropTarget())
        {
            if (ImGui::IsMouseHoveringRect(image_min, image_max))
            {
                draw_list->AddRect(image_min, image_max,
                                   ImGui::GetColorU32(ImVec4(0.26f, 0.59f, 0.98f, 0.67f)), 4.0f, 0,
                                   2.0f);
                ...
                if (payload)
                {
                    ...
                    std::string extension =
                        std::filesystem::path(dropped_path).extension().string();
        ...
        {
            ImGui::SetCursorScreenPos(image_min);
            ImGui::Image((void*)(intptr_t)props.texture->get_texture_id(), preview_size);
        ...
        else
        {
            ...
            ImVec2      text_pos  = ImVec2(image_min.x + (preview_size.x - text_size.x) * 0.5f,
    ...
    if (props.texture)
    {
        ImGui::Text("Size: %dx%d", props.texture->get_width(), props.texture->get_height());

        ImGui::SameLine();
        ImGui::TextDisabled("| %s", std::filesystem::path(props.path).extension().string().c_str());
    }
...
bool UI::is_right_file(const std::filesystem::path& path)
{
    ...
    if (ext == ".png" || ext == ".jpg" || ext == ".jpeg" || ext == ".bmp" || ext == ".tga" ||
        ext == ".hdr" || ext == ".tiff" || ext == ".tif" || ext == ".webp" || ext == ".gif" ||
        ext == ".psd" || ext == ".svg")
        return true;
...
bool UI::is_image(const std::filesystem::path& path)
{
    ...
    return ext == ".png" || ext == ".jpg" || ext == ".jpeg" || ext == ".bmp" || ext == ".tga" ||
           ext == ".hdr" || ext == ".tiff" || ext == ".tif" || ext == ".webp";
}
...
void UI::draw_folder_node(const std::filesystem::path& directory, int depth)
{
    ...
    for (const auto& entry : entries)
    {
        ...
        if (entry.is_directory())
        {
            ...
            bool is_open =
                ImGui::TreeNodeEx((std::string(ICON_FA_FOLDER) + "  " + name).c_str(), flags);
        ...
        {
            ImGuiTreeNodeFlags flags = ImGuiTreeNodeFlags_Framed | ImGuiTreeNodeFlags_Leaf |
            ...
            ImGui::TreeNodeEx((std::string(file_icon) + "  " + name).c_str(), flags);
...
const char* UI::get_file_icon(const std::string& extension)
{
    ...
    // Code files
    if (ext == ".cpp" || ext == ".h" || ext == ".hpp" || ext == ".c" || ext == ".cs" ||
        ext == ".java")
...
void UI::draw_animation(Entity entity)
{
    ...
    ImGui::Text(
        "Frame: %d / %s", anim.current_frame,
        anim.current_animation.empty()
            ? "N/A"
    ...
    ImGui::InputTextWithHint("##NewAnimName", "New Animation Name", new_anim_name,
                             IM_ARRAYSIZE(new_anim_name));
    ImGui::SameLine();
    ...
    for (const auto& name : animation_names)
    {
        ...
        bool anim_open = ImGui::TreeNodeEx(name.c_str(), ImGuiTreeNodeFlags_Framed |
        ...
        float button_x =
            start_x + available_width - button_width - ImGui::GetStyle().FramePadding.x * 2;

        float saved_cursor_y = ImGui::GetCursorPosY();
        float saved_cursor_x = ImGui::GetCursorPosX();

        ImGui::SetCursorPosX(button_x);
        ImGui::SetCursorPosY(saved_cursor_y - ImGui::GetFrameHeight() -
...
void UI::draw_frame_selector(Entity entity, const std::string& current_animation)
{
    ...
    extra_settings.button_controls = [&](const Button_Control_Value& details)
    {
        ...
        bool is_selected = std::find(selected_frames.begin(), selected_frames.end(), cell_index) !=
                           selected_frames.end();

        vec2f cell_min = details.screen_pos + vec2f(details.tile_id.x * details.size.x,
                                                    details.tile_id.y * details.size.y) *
                                                  details.scale;
        vec2f cell_max = details.screen_pos + vec2f((details.tile_id.x + 1) * details.size.x,
                                                    (details.tile_id.y + 1) * details.size.y) *
                                                  details.scale;

        if (is_selected)
        {
            details.draw_list->AddRectFilled(cell_min.to_imvec2(), cell_max.to_imvec2(),
                                             IM_COL32(0, 255, 0, 80));
            details.draw_list->AddRect(cell_min.to_imvec2(), cell_max.to_imvec2(),
                                       IM_COL32(0, 255, 0, 200), 0.0f, 0, 3.0f);
        }
        ...
        if (ImGui::InvisibleButton("##Cell", details.button_size.to_imvec2()))
        {
            ...
            else if (shift_down && !selected_frames.empty())
            {
                ...
                {
                    if (std::find(selected_frames.begin(), selected_frames.end(), i) ==
                        selected_frames.end())
                    {
        ...
        if (ImGui::IsItemHovered())
        {
            ...
            ImGui::Text("  Position: (%.0f, %.0f)", details.tile_id.y * cell_width,
            details.tile_id.x * cell_height); ImGui::Text("  Size: %.0fx%.0f", cell_width,
            cell_height);*/
            if (is_selected)
            {
    ...
    extra_settings.controls_panel = [&]()
    {
        ...
        ImGui::Text("Selected: %d/%d frames", (int)selected_frames.size(),
                    total_grid.x * total_grid.y);
...
void UI::draw_tilemap_register(Entity entity)
{
    ...
    {
        custom_popup(
            "TileSet Register", "TileSet Register",
            [&]() mutable
            {
                ...
                ImGui::BeginChild("TextureColumn", ImVec2(left_width, 0), true);
                {
                    ...
                    ImGui::BeginChild("TextureContainer", ImVec2(0, 0), true,
                                      ImGuiWindowFlags_NoScrollbar |
                                          ImGuiWindowFlags_NoScrollWithMouse);
                    {
                        ...
                        if (is_dragging)
                        {
                            ...
                            {
                                vec2f start = Math::screen_to_world(
                                    last_mouse_pos, Math::get_float_rect(view_size, view_center),
                                    container);
                                vec2f end = Math::screen_to_world(
                                    current_mouse_pos, Math::get_float_rect(view_size, view_center),
                                    container);
                        ...
                        if (is_hovering_container && ImGui::GetIO().MouseWheel != 0)
                        {
                            ...
                            vec2f world_before_zoom = Math::screen_to_world(
                                current_mouse_pos, Math::get_float_rect(view_size, view_center),
                                container);
                            view_size *= scale_factor;
                            vec2f world_after_zoom = Math::screen_to_world(
                                current_mouse_pos, Math::get_float_rect(view_size, view_center),
                                container);
                        ...
                        vec2f screen_pos = Math::world_to_screen(
                            world_pos, Math::get_float_rect(view_size, view_center), container);
                        vec2f screen_size =
                            Math::world_size_to_screen_size(texture_size, view_size, container);
                        ...
                        { // Vertical
                            float px =
                                screen_pos.x + x * tile_size.x * (screen_size.x / texture_size.x);
                        ...
                        { // Horizontal
                            float py =
                                screen_pos.y + y * tile_size.y * (screen_size.y / texture_size.y);
                            draw_list->AddLine(ImVec2(screen_pos.x, py),
                                               ImVec2(screen_pos.x + screen_size.x, py),
                                               IM_COL32(255, 255, 255, 100), 2.0f);
                        }

                        vec2f button_size =
                            Math::world_size_to_screen_size(tile_size, view_size, container);
                        for (int x = 0; x < total_dim.x; x++)
                        {
                            for (int y = 0; y < total_dim.y; y++)
                            {
                                ...
                                bool is_selected =
                                    std::find(selected_tiles.begin(), selected_tiles.end(),
                                              tile_id) != selected_tiles.end();

                                bool is_solid = std::find(solid_tiles.begin(), solid_tiles.end(),
                                                          tile_id) != solid_tiles.end() &&
                                                is_selected;

                                vec2f scale = screen_size / texture_size;
                                vec2f cell_min =
                                    screen_pos + vec2f(x * tile_size.x, y * tile_size.y) * scale;
                                vec2f cell_max =
                                    screen_pos +
                                    vec2f((x + 1) * tile_size.x, (y + 1) * tile_size.y) * scale;

                                ImU32 selected_color = is_selected ? IM_COL32(200, 200, 200, 50)
                                                                   : IM_COL32(0, 0, 0, 150);

                                ImU32 solid_color =
                                    is_solid ? IM_COL32(200, 50, 50, 100) : IM_COL32(0, 0, 0, 150);

                                draw_list->AddRectFilled(cell_min.to_imvec2(), cell_max.to_imvec2(),
                                                         selected_color);

                                if (is_for_solid)
                                    draw_list->AddRectFilled(cell_min.to_imvec2(),
                                                             cell_max.to_imvec2(), solid_color);
                                ...
                                if (ImGui::InvisibleButton("##Cell", button_size.to_imvec2()))
                                {
                                    ...
                                    if (control)
                                    {
                                        ...
                                        {
                                            auto selected =
                                                std::find(selected_tiles.begin(),
                                                          selected_tiles.end(), tile_id);
                                            if (selected != selected_tiles.end())
                                            {
                                                auto it = std::find(solid_tiles.begin(),
                                                                    solid_tiles.end(), tile_id);
                                    ...
                                    else if ((shift && !selected_tiles.empty() && !is_for_solid) ||
                                             (shift && !solid_tiles.empty() && is_for_solid))
                                    {
                                        if (!is_for_solid)
                                        {
                                ...
                                if (ImGui::IsItemHovered())
                                {
                                    ...
                                    ImGui::Text("Position: (%d, %d)", x * (int)tile_size.x,
                                                y * (int)tile_size.y);
                ...
                ImGui::BeginChild("Control", ImVec2(right_width, 0), true);
                {
                    ...
                    if (ImGui::Button("Register Tiles", ImVec2(button_width, button_height)) &&
                        !selected_tiles.empty())
                    {
                        if (!entity.has_component<TileSet_Component>())
                        {
                        ...
                        for (const auto& ids : selected_tiles)
                        {
                            ...
                            def.is_solid = std::find(solid_tiles.begin(), solid_tiles.end(), ids) !=
                                           solid_tiles.end();
...
void UI::draw_autotiling_register(Entity entity)
{
    ...
    }
    {
        ...
        if (ImGui::BeginCombo("Current Set", active_set.empty() ? "None" : active_set.c_str()))
        {
    ...
    if (ImGui::Button("Edit Set", ImVec2(width, 30.0f)))
    {
        if (!entity.has_component<Texture_Component>() ||
            ...
            !entity.has_component<TileSet_Component>() ||
            !entity.has_component<AutoTiling_Component>())
            return;
    ...
    {
        custom_popup(
            "AutoTiling Register", "AutoTiling Register",
            [&]() mutable
            {
                auto&       texture      = entity.get_component<Texture_Component>().texture;
                vec2f       texture_size = texture->get_size();
                            ...
                            for (int y = 0; y < total_dim.y; y++)
                            {
                                ...
                                for (int sy = 0; sy < 3; sy++)
                                {
                                    for (int sx = 0; sx < 3; sx++)
                                    {
                                        ...
                                        ImU32 color = selected ? IM_COL32(255, 0, 0, 180)
                                                               : IM_COL32(50, 50, 50, 0);

                                        draw_list->AddRectFilled(sub_min.to_imvec2(),
                                                                 sub_max.to_imvec2(), color);

                                        ImGui::PushID((sy * 3 + sx));
                                        ImGui::SetCursorScreenPos(sub_min.to_imvec2());

                                        if (ImGui::InvisibleButton("##autobit",
                                                                   sub_size.to_imvec2()))
                                        {
                                            mask ^= bit;
                                        }
                ...
                ImGui::BeginChild("Control", ImVec2(right_width, 0), true);
                {
                    ...
                    ImGui::Dummy(ImVec2(0.0f, 2.0f));
                    if (ImGui::BeginPopupModal("Set Name Exists", NULL,
...
bool UI::draw_tilemap_selector(Entity entity, vec2u& id, std::string& set_name, bool& use_autotile)
{
    ...
    if (!entity.has_component<Tile_Component>() || !entity.has_component<Texture_Component>() ||
        !entity.get_component<Texture_Component>().texture ||
        !entity.has_component<TileSet_Component>())
    {
        return false;
        ...
        ImGui::BeginChild("TextureColumn", ImVec2(0, 0), true);
        {
                              ...
                              ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);
            {
                ...
                { // On Resize
                    ...
                    vec2f container           = ImGui::GetContentRegionAvail();
                    vec2f container_start_pos = ImGui::GetCursorScreenPos();
                    bool  is_hovering_container =
                        ImGui::IsMouseHoveringRect(container_start_pos.to_imvec2(),
                                                   (container_start_pos + container).to_imvec2());
                    ...
                    for (int x = 0; x < total_dim.x; x++)
                    {
                        for (int y = 0; y < total_dim.y; y++)
                        {
                            ...
                            bool is_registered =
                                std::find(registered_tiles.begin(), registered_tiles.end(),
...
bool UI::texture_selector(Entity entity, uint_rect& texture_rect)
{
    ...
    if (!entity.has_component<TextureRect_Component>() ||
        !entity.has_component<Texture_Component>() ||
        !entity.get_component<Texture_Component>().texture)
        return false;
    ...
    if (ImGui::Begin("Texture Selector", &window_open, flags))
    {
        ...
        ImGui::BeginChild("TextureColumn", ImVec2(0, 0), true);
        {
                              ...
                              ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);
            {
                ...
                vec2f container_start_pos   = ImGui::GetCursorScreenPos();
                bool  is_hovering_container = ImGui::IsMouseHoveringRect(
                    container_start_pos.to_imvec2(), (container_start_pos + container).to_imvec2());
                ...
                if (is_hovering_container && ImGui::GetIO().MouseWheel != 0)
                {
                    float scale_factor      = (ImGui::GetIO().MouseWheel > 0) ? 0.9f : 1.1f;
                    vec2f world_before_zoom = Math::screen_to_world(
                        current_mouse_pos, Math::get_float_rect(view_size, view_center), container);
                    view_size *= scale_factor;
                ...
                vec2f screen_pos = Math::world_to_screen(
                    image_pos, Math::get_float_rect(view_size, view_center), container);
                vec2f screen_size =
                    Math::world_size_to_screen_size(texture_size, view_size, container);
                ImGui::SetCursorPos(screen_pos.to_imvec2());
                ...
                draw_list->AddRectFilled((screen_pos - padding_size).to_imvec2(),
                ...
                draw_list->AddRectFilled(screen_pos.to_imvec2(),
                ...
                if (is_hovered && ImGui::IsMouseClicked(0))
                {
                    ...
                    vec2f world_pos    = Math::screen_to_world(
                        current_mouse_pos, Math::get_float_rect(view_size, view_center), container);
                ...
                if (is_hovered && ImGui::IsMouseDown(0))
                {
                    ...
                    vec2f start_pos{(std::min(starting_pixel.x, ending_pixel.x)),
                                    (std::min(starting_pixel.y, ending_pixel.y))};
                    vec2f end_pos{(std::max(starting_pixel.x, ending_pixel.x)) + 1.0f,
                                  (std::max(starting_pixel.y, ending_pixel.y)) + 1.0f};

                    vec2f start = Math::world_to_screen(
                        start_pos + image_pos, Math::get_float_rect(view_size, view_center),
                        container);
                    vec2f end = Math::world_to_screen(end_pos + image_pos,
                                                      Math::get_float_rect(view_size, view_center),
                                                      container);

                    vec2f uv0 = starting_pixel / texture_size;
                    vec2f uv1 = (ending_pixel + 1.0f) / texture_size;

                    vec2f uv_min{std::min(uv0.x, uv1.x), std::min(uv0.y, uv1.y)};
                    vec2f uv_max{std::max(uv0.x, uv1.x), std::max(uv0.y, uv1.y)};

                    draw_list->AddImage(texture_id, (window_pos + start).to_imvec2(),
                                        (window_pos + end).to_imvec2(), uv_min.to_imvec2(),
                                        uv_max.to_imvec2());

                    draw_list->AddRect((window_pos + start).to_imvec2(),
...
/*void UI::draw_script_selector(Entity entity)
{
    ...
    float button_width = (ImGui::GetContentRegionAvail().x - ImGui::GetStyle().ItemSpacing.x)
/ 2.0f; if (ImGui::Button("Create New", ImVec2(button_width, 0)))
    {
    ...
    {
        auto full_path = FileDialogs::open_file("Lua Scripts(*.lua)\0 * .lua\0All Files(*.*)\0 *
.*\0"); if (!full_path.empty())
        {
...
void UI::draw_loaded_script_panel(Entity entity, Script_Component& props)
{
    ...
    if (ImGui::BeginPopupContextItem("##script_context_menu", ImGuiPopupFlags_MouseButtonRight))
    {
        ...
        if (ImGui::MenuItem(ICON_FA_CODE "  Open in VS Code", nullptr, false, !props.path.empty()))
        {
            ...
            std::string full_path =
                project->get_directory() + project->get_scripts_directory() + props.path;
            open_in_vscode(full_path);
        }

        if (ImGui::MenuItem(ICON_FA_LINK "  Open Containing Folder", nullptr, false,
                            !props.path.empty()))
        {
            auto        project = Project::get_active_project();
            std::string full_path =
    ...
    if (ImGui::BeginDragDropTarget())
    {
        const ImGuiPayload* payload = ImGui::AcceptDragDropPayload(
            "CONTENT_BROWSER_ITEM", ImGuiDragDropFlags_AcceptNoDrawDefaultRect);
...
void UI::draw_script_drop_zone(Entity entity, Script_Component& props)
{
    ...
    ImGui::SetCursorPos(
        ImVec2((window_size.x - content_size.x) * 0.5f, (window_size.y - content_size.y) * 0.5f));
...
void UI::draw_script_actions(Entity entity, Script_Component& props, bool has_script)
{
    ...
    {
        auto full_path =
            FileDialogs::open_file("Lua Scripts(*.lua)\0 * .lua\0All Files(*.*)\0 * .*\0");
        if (!full_path.empty())
        {
...
bool UI::draw_button(const GUI_Button& btn)
{
    ...
    if (btn.enabled)
    {
        clicked = ImGui::Button(btn.label.c_str(), btn.size.to_imvec2());
    }
    else
    {
        ImGui::BeginDisabled();
                ...
                if (can_create)
                {
                    if (!path.empty())
                    {
                        ...
                        std::string relative_path;
                        {
                            auto        project = Project::get_active_project();
                            std::string base_path =
                                project->get_directory() + project->get_scripts_directory() + "/";
...
void UI::custom_popup(const std::string& popup_id, const std::string& popup_name,
                      std::function<void()> draw_content, std::function<void()> close)
{
    ...
    ImGui::SetNextWindowPos(ImVec2(center.x - modal_size.x * 0.5f, center.y - modal_size.y * 0.5f),
                            ImGuiCond_Appearing);
    ...
    if (ImGui::BeginPopupModal(popup_id.c_str(), NULL, flag))
    {
        ...
        draw_list->AddRectFilled(
            window_pos, ImVec2(window_pos.x + window_size.x, window_pos.y + title_bar_height),
            title_bar_color);
