...
void ScenePanel::draw_create_object()
{
  ...
  model.draw_content = [this]()
  {
    ...
    float cursor_y =
        ImGui::GetCursorPosY() + ImGui::GetContentRegionAvail().y - button_size.y - spacing;
...
{
  if (!entity || !entity.has_component<Tag_Component>())
    return;
  auto&    tag       = entity.get_component<Tag_Component>();
  ...
  if (!m_hierarchy_state.filter_text.empty())
  {
    ...
    std::transform(entity_name_lower.begin(),
                   entity_name_lower.end(),
                   entity_name_lower.begin(),
                   ::tolower);
  ...
  bool is_expanded =
      m_hierarchy_state.expanded_nodes.find(entity_id) != m_hierarchy_state.expanded_nodes.end();
  ...
  bool should_start_drag = ImGui::IsItemActive() && ImGui::IsMouseDragging(ImGuiMouseButton_Left);
...
void ScenePanel::draw_drop_zone_between(Entity       entity,
                                        DropPosition position,
                                        int          level,
                                        Entity       parent)
{
  ImGui::PushID(("drop_zone_" + std::to_string((uint32_t)entity.get_id()) + "_" +
  ...
  if (is_hovered && m_hierarchy_state.dragged_entity)
  {
    ...
    draw_list->AddLine(line_start, line_end, IM_COL32(0, 150, 255, 255), 2.0f);
  }
...
void ScenePanel::insert_entity_before(EntityID dragged_id, EntityID target_id)
{
  ...
  {
    auto& root_entities = m_scene->get_root_entity();
    root_entities.erase(std::remove(root_entities.begin(), root_entities.end(), dragged_id),
                        root_entities.end());
  }
...
void ScenePanel::draw_hierarchy_toolbar()
{
  ImGui::BeginChild("HierarchyToolbar", ImVec2(0, 35), false);
  {
    ...
    // Filter toggle
    if (draw_toolbar_button(m_hierarchy_state.show_filter ? ICON_FA_FILTER
    ...
    if (m_selected_entity && m_selected_entity.has_component<Tag_Component>())
    {
      ...
      if (draw_toolbar_button(vis.visible ? ICON_FA_EYE : ICON_FA_EYE_SLASH,
    ...
    if (m_selected_entity && m_selected_entity.has_component<Tag_Component>())
    {
      ...
      if (draw_toolbar_button(lock.locked ? ICON_FA_LOCK : ICON_FA_UNLOCK,
...
void ScenePanel::reparent_entity(EntityID child_id, EntityID new_parent_id)
{
  ...
  if (child_tag.parent)
  {
    ...
    auto  it = std::find(old_parent_tag.children.begin(), old_parent_tag.children.end(), child);
  ...
  {
    auto& root_entities = m_scene->get_root_entity();
    root_entities.erase(std::remove(root_entities.begin(), root_entities.end(), child_id),
                        root_entities.end());
  }
...
void ScenePanel::make_root_entity()
{
  ...
  if (tag.parent && tag.parent.get_id() != INVALID_ENTITY)
  {
    ...
    auto  it = std::find(parent_tag.children.begin(), parent_tag.children.end(), m_selected_entity);
    if (it != parent_tag.children.end())
    {
...
void ScenePanel::draw_selected_text()
{
  ...
  if (m_selected_entity.has_component<Transform_Component>())
  {
    ...
    transform.position = Math::world_to_screen(trans.position,
                                               EditorLayer::get().get_float_rect(),
  ...
  for (auto id : view)
  {
    ...
    for (const auto& [id, shape] : shapes.shapes)
    {
      ...
      switch (shape.shape_type)
      {
        ...
        {
          auto rect_size =
              Math::world_size_to_screen_size(shape.size, camera.get_size(), viewport_size);
          rect.size       = rect_size;
          ...
          shape_transform.position = Math::world_to_screen(shape_transform.position + shape.offset,
                                                           camera.get_float_rect(),
                                                           viewport_size);
          shape_transform.rotation += shape.rotation;
          Renderer2D::draw_rectangle(rect, shape_transform, entity_id);
          break;
        }
        case ShapeType::Circle:
        {
          auto circle_size  = Math::world_size_to_screen_size({shape.radius * 2, shape.radius * 2},
                                                             camera.get_size(),
                                                             viewport_size);
...
void ScenePanel::tile_map_draw()
{
  ...
  vec2i current_tile = {(int)std::floor((current_mouse.x - props.offset.x) / props.size.x),
                        (int)std::floor((current_mouse.y - props.offset.y) / props.size.y)};

  if (is_left && !is_right)
  {
    m_settings = TileMap_Settings::Paint;
    if (!dragging && (m_paint_settings == TileMap_Paint_Settings::Rectangle ||
...
void ScenePanel::paint_tiles(TileSet_Component& tile_set, const vec2i& pos)
{
  ...
  placed.tile_id = (def != it->second.tile_bitmask.end()) ? def->second
...
void ScenePanel::update_neighbour(TileSet_Component& tile_set, const vec2i& start)
{
  ...
  if (set_id == std::numeric_limits<uint16_t>::max())
    return;

  auto& comps   = m_selected_entity.get_component<AutoTiling_Component>();
  auto  auto_it = comps.auto_tiles.find(m_active_set);
  if (auto_it == comps.auto_tiles.end())
    return;
