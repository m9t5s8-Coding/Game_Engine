...
namespace ag
{
static const uint16_t bit_lookup[3][3] = {{TL, T, TR}, {L, 0, R}, {BL, B, BR}};
...
ScenePanel::ScenePanel(const AG_ref<Scene>& scene)
{
    ...
    NodeFactory::properties_map[NodeType::Rectangle] =
        NodeProperties::rectangle_2D;
    ...
    NodeFactory::properties_map[NodeType::AnimatedSprite2D] =
        NodeProperties::animated_sprite_2D;
    ...
    NodeFactory::properties_map[NodeType::TextureButton] =
        NodeProperties::texture_button_2D;
...
void ScenePanel::on_update()
{
    ...
    if (has_selected_entity())
    {
        if (m_selected_entity.has_component<Tag_Component>() &&
            EditorLayer::get().is_viewport_hovered())
        {
            const auto& tag = m_selected_entity.get_component<Tag_Component>();
            if (tag.node_type == NodeType::TileMap) tile_map_draw();
        }
...
void ScenePanel::on_event(Event& e)
{
    ...
    dispatcher.Dispatch<TextInputEvent>(
        AERO_BIND_EVENT_FN(ScenePanel::on_text_input));
    dispatcher.Dispatch<KeyPressedEvent>(
        AERO_BIND_EVENT_FN(ScenePanel::on_key_pressed));
    dispatcher.Dispatch<MouseButtonPressedEvent>(
        AERO_BIND_EVENT_FN(ScenePanel::on_mouse_pressed));
}

void ScenePanel::set_selected_entity(Entity entity)
{
    if (has_selected_entity() &&
        m_selected_entity.has_component<Text_Editor_State>())
    {
...
void ScenePanel::on_imgui_render()
{
    ...
    if (m_show_create_panel) draw_create_object();
    ...
    if (m_selected_entity)
    {
        ...
        {
            auto& rects =
                m_selected_entity.get_component<TextureRect_Component>();
            auto& texture =
                m_selected_entity.get_component<Texture_Component>().texture;
            ...
            if (texture)
            {
                if (UI::texture_selector(m_selected_entity, texture_rect))
                {
                    ...
                    if (m_selected_entity
                            .has_component<Textured_Button_Component>())
                    {
                        auto& comps =
                            m_selected_entity
                                .get_component<Textured_Button_Component>();
                    ...
                    {
                        auto& sizes = m_selected_entity
                                          .get_component<Render2D_Component>();
...
void ScenePanel::draw_node_hierarchy(Entity entity, int level)
{
    ...
    if (m_selected_entity == entity) flags |= ImGuiTreeNodeFlags_Selected;

    if (tag.children.empty()) flags |= ImGuiTreeNodeFlags_Leaf;

    ImGui::Indent(level * 10.0f);

    // Draw the tree node
    bool opened = ImGui::TreeNodeEx((void*)(uint64_t)entity.get_id(), flags,
                                    "%s", tag.name.c_str());
...
void ScenePanel::draw_create_object()
{
    ...
    PopUpModel model = {.name = "Create Objects",
                        .id = "##CreateObjects",
                        .confirm_name = "Create",
                        .close_name = "Cancel",
                        .window_size = {450, 580}};

    model.on_close = [this]() { this->m_state.should_close = true; };
    ...
    model.draw_content = [this]()
    {
        ...
        float spacing = 10.0f;
        vec2f button_size = {(available_width - (spacing * 3)) * 0.5f, 35.0f};

        float cursor_y = ImGui::GetCursorPosY() +
                         ImGui::GetContentRegionAvail().y - button_size.y -
                         spacing;
...
void ScenePanel::draw_entity_node(Entity entity, int level)
{
    ...
    if (should_start_drag)
    {
        if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_SourceAllowNullID))
        {
            ...
            ImGui::SetDragDropPayload("ENTITY_NODE", &entity_id,
                                      sizeof(EntityID));
    ...
    if (node_open)
    {
        ...
        {
            draw_drop_zone_between(Entity{}, DropPosition::LastChild, level + 1,
                                   entity);
        }
                                        ...
                                        int level, Entity parent)
{
    ...
    if (is_hovered && m_hierarchy_state.dragged_entity)
    {
        ...
        draw_list->AddLine(line_start, line_end, IM_COL32(0, 150, 255, 255),
                           2.0f);
    }
...
void ScenePanel::insert_entity_before(EntityID dragged_id, EntityID target_id)
{
    ...
    if (dragged_tag.parent)
    {
        ...
        parent_tag.children.erase(
            std::remove(parent_tag.children.begin(), parent_tag.children.end(),
                        dragged),
    ...
    if (target_tag.parent)
    {
        ...
        auto it = std::find(parent_tag.children.begin(),
                            parent_tag.children.end(), target);
    ...
    else
    {
        auto& root_entities = m_scene->get_root_entity();
        auto it =
            std::find(root_entities.begin(), root_entities.end(), target_id);
...
void ScenePanel::insert_entity_as_last_child(EntityID dragged_id,
                                             EntityID parent_id)
{
...
bool ScenePanel::is_parent(Entity parent, Entity entity)
{
    ...
    while (parent_id)
    {
        if (parent_id == parent.get_id()) return true;
...
void ScenePanel::draw_hierarchy_toolbar()
{
    ImGui::BeginChild("HierarchyToolbar", ImVec2(0, 35), false);
    {
        ...
        // Filter toggle
        if (draw_toolbar_button(m_hierarchy_state.show_filter
                                    ? ICON_FA_FILTER
        ...
        // Visibility toggle
        if (m_selected_entity &&
            m_selected_entity.has_component<Tag_Component>())
        {
...
void ScenePanel::draw_entity_context_menu(Entity entity)
{
    ...
    if (entity.has_component<Tag_Component>())
    {
        ...
        if (ImGui::MenuItem(vis.visible ? ICON_FA_EYE_SLASH "  Hide"
                                        : ICON_FA_EYE "  Show"))
        {
            vis.visible = !vis.visible;
            m_scene->set_save_required(true);
    ...
    if (entity.has_component<Tag_Component>())
    {
        ...
        if (ImGui::MenuItem(lock.locked ? ICON_FA_UNLOCK "  Unlock"
                                        : ICON_FA_LOCK "  Lock"))
        {
            lock.locked = !lock.locked;
            m_scene->set_save_required(true);
...
void ScenePanel::draw_hierarchy_context_menu(Entity* entity)
{
    ...
    {
        m_hierarchy_state.auto_expand_to_selection =
            !m_hierarchy_state.auto_expand_to_selection;
    }
}
...
const char* ScenePanel::get_node_icon(Entity entity)
{
    ...
    switch (type)
    {
        ...
        case NodeType::TextureButton:
            return ICON_FA_IMAGE_PORTRAIT;
        case NodeType::CollisionShape:
...
void ScenePanel::expand_all_nodes()
{
    ...
    {
        m_hierarchy_state.expanded_nodes.insert(
...
void ScenePanel::reparent_entity(EntityID child_id, EntityID new_parent_id)
{
    ...
    AERO_INFO("Reparented {} to {}", child_tag.name, parent_tag.name);
}
...
void ScenePanel::make_root_entity()
{
    ...
    auto child_transform =
        Transform_Component::get_world_transform(m_selected_entity);
    ...
    {
        auto& transform =
            m_selected_entity.get_component<Transform_Component>();
...
void ScenePanel::create_selected_object(NodeType type)
{
    ...
    if (it == NodeFactory::nodes.end() ||
        create_it == NodeFactory::create_map.end())
    {
        AERO_WARN("Cannot create object: type {} not registered",
    ...
    {
        auto& root_entity = m_scene->get_root_entity();
        root_entity.push_back(new_entity.get_id());
    }
...
{
    if (m_selected_entity.get_id() == INVALID_ENTITY) return;
    ...
    if (m_selected_entity.has_component<Transform_Component>())
    {
        ...
        transform.position = Math::world_to_screen(
            trans.position, EditorLayer::get().get_float_rect(),
            EditorLayer::get().get_viewport_size());
        Text text;
        text.text = name;
...
void ScenePanel::draw_collision_shapes()
{
    ...
    for (auto id : view)
    {
        ...
        for (const auto& [id, shape] : shapes.shapes)
        {
            auto shape_transform = transform;
            switch (shape.shape_type)
            {
                case ShapeType::Rectangle:
                {
                    auto rect_size = Math::world_size_to_screen_size(
                        shape.size, camera.get_size(), viewport_size);
                    rect.size = rect_size;
                    rect.mode = RenderMode::Screen;
                    rect.fill_color = Color(80, 180, 255, 40);

                    if (shape.is_sensor)
                        rect.border_color = Color(200, 30, 50, 220);
                    else
                        rect.border_color = Color(30, 200, 50, 220);

                    rect.border_thickness = -2.0f;

                    shape_transform.position = Math::world_to_screen(
                        shape_transform.position + shape.offset,
                        camera.get_float_rect(), viewport_size);
                    shape_transform.rotation += shape.rotation;
                    Renderer2D::draw_rectangle(rect, shape_transform,
                                               entity_id);
                    break;
                }
                case ShapeType::Circle:
                {
                    auto circle_size = Math::world_size_to_screen_size(
                        {shape.radius * 2, shape.radius * 2}, camera.get_size(),
                        viewport_size);
                    circle.size = circle_size;
                    circle.mode = RenderMode::Screen;
                    circle.fill_color = Color(80, 180, 255, 40);
                    if (shape.is_sensor)
                        circle.border_color = Color(200, 30, 50, 220);
                    else
                        circle.border_color = Color(30, 200, 50, 220);
                    ...
                    shape_transform.position = Math::world_to_screen(
                        ...
                        camera.get_float_rect(), viewport_size);
                    shape_transform.rotation += shape.rotation;
                    Renderer2D::draw_circle(circle, shape_transform, entity_id);
                    break;
                }
                default:
                {
...
{
    if (!m_selected_entity ||
        !m_selected_entity.has_component<Transform_Component>() ||
        !EditorLayer::get().is_viewport_hovered())
        return;
    ...
    switch (m_current_transform_setting)
    {
        ...
        default:
            AERO_CORE_ERROR("No sucn Transform_Component Setting!");
            break;
    }
...
{
    auto& position =
        m_selected_entity.get_component<Transform_Component>().position;
    if (!m_move_flag)
    {
...
{
    auto& rotation =
        m_selected_entity.get_component<Transform_Component>().rotation;
    auto position =
        Transform_Component::get_world_transform(m_selected_entity).position;
    ...
    rotation += Math::angle_betn_3points(m_last_mouse_position, position,
...
void ScenePanel::draw_selection_box()
{
    ...
    {
        const auto& render2d =
            m_selected_entity.get_component<Render2D_Component>();
        Transform_Component transform =
            Transform_Component::get_world_transform(m_selected_entity);
...
void ScenePanel::delete_entity()
{
    ...
    if (m_selected_entity == m_properties_entity)
...
bool ScenePanel::texture_selector(const AG_ref<Texture2D>& texture,
                                  ...
                                  uint_rect& texture_rect)
{
    ...
    ImGui::Begin("Texture Selector");
    {
        ...
        ImGui::Image((ImTextureID)(uintptr_t)texture->get_texture_id(),
                     image_size);
            ...
            ImGui::IsMouseClicked(ImGuiMouseButton_Left))
        {
            ...
            texture_rect = uint_rect(tile_x * tile_size.x, tile_y * tile_size.y,
                                     tile_size);
...
{
    if (!has_selected_entity()) return false;

    bool control = Keyboard::is_key_pressed(Key::LeftControl) ||
                   Keyboard::is_key_pressed(Key::RightControl);
    bool shift = Keyboard::is_key_pressed(Key::LeftShift) ||
                 Keyboard::is_key_pressed(Key::RightShift);

    // Typing text
    if (m_selected_entity.has_component<Text_Editor_State>() &&
        m_selected_entity.get_component<Text_Editor_State>().active &&
        m_selected_entity.has_component<Text_Component>())
    {
        ...
        else if (e.get_key_code() == Key::Right &&
        ...
        else if (e.get_key_code() == Key::Backspace)
        {
            if (control)
            {
                if (state.caret_index == 0) return true;
                ...
                props.text.erase(props.text.begin() + delete_start,
                                 props.text.begin() + state.caret_index);
        ...
        else if (e.get_key_code() == Key::V)
        {
            if (control)
            {
                std::string clipboard_text =
                    Application::get().get_window().get_clipboard_string();
                props.text.insert(props.text.begin() + state.caret_index,
                                  clipboard_text.begin(), clipboard_text.end());
                state.blink_timer = 0.0f;
    ...
    switch (e.get_key_code())
    {
        ...
        case Key::R:
            ...
            return true;
        case Key::X:
            m_current_transform_axis = TransformAxis::X;
            return true;
        case Key::Y:
            m_current_transform_axis = TransformAxis::Y;
            return true;
        case Key::Escape:
        {
...
bool ScenePanel::on_mouse_pressed(MouseButtonPressedEvent& e) { return false; }
...
bool ScenePanel::on_text_input(TextInputEvent& e)
{
        ...
        m_selected_entity.has_component<Text_Editor_State>())
    {
        ...
        text.text.insert(text.text.begin() + state.caret_index,
                         e.get_character());
...
{
    m_scene->m_registry.sort<Tag_Component>(
        [](const Tag_Component& a, const Tag_Component& b)
    ...
    for (auto entityID : view)
    {
        ...
        auto& tag = entity.get_component<Tag_Component>();
        if (tag.parent.get_id() != INVALID_ENTITY) continue;

        for (auto e = tag.children.rbegin(); e != tag.children.rend(); ++e)
        {
            if (check_if_clicked(*e)) return true;
        }

        if (check_if_clicked(entity)) return true;
    }
...
{
    if (!entity.has_component<Transform_Component>()) return false;

    if (!entity.has_component<Render2D_Component>()) return false;

    auto& transform = entity.get_component<Transform_Component>();
    auto& render2d = entity.get_component<Render2D_Component>();

    vec2f position =
        transform.position - (render2d.size * transform.scale) * 0.5f;
...
{
    bool has_tileset = m_selected_entity.has_component<Tile_Component>() &&
                       m_selected_entity.has_component<TileSet_Component>();
    bool has_solidset = m_selected_entity.has_component<Tile_Component>() &&
                        m_selected_entity.has_component<SolidSet_Component>();
    ...
    static vec2i previous_tile = {std::numeric_limits<int>::max(),
    ...
    if ((!is_left && !is_right) || (is_left && is_right))
    {
        ...
        dragging = false;
        temp_tiles.clear();
        previous_tile = {std::numeric_limits<int>::max(),
                         std::numeric_limits<int>::max()};
        return;
    }
    ...
    if (is_left && !is_right)
    {
        ...
        if (!dragging &&
            (m_paint_settings == TileMap_Paint_Settings::Rectangle ||
             m_paint_settings == TileMap_Paint_Settings::Line ||
             m_paint_settings == TileMap_Paint_Settings::Fill))
        {
...
void ScenePanel::paint_eraser_tiles_helper(TileSet_Component& tile_set,
                                           const vec2i& pos)
{
    switch (m_settings)
    {
...
void ScenePanel::solid_paint_erase_helper(SolidSet_Component& solid_set,
...
void ScenePanel::paint_tiles(TileSet_Component& tile_set, const vec2i& pos)
{
    ...
    if (it == comps.auto_tiles.end()) return;
    ...
    {
        if (placed.tile_id != id->second) placed.tile_id = id->second;
    }
...
void ScenePanel::erase_tiles(TileSet_Component& tile_set, const vec2i& pos)
{
    ...
    if (it == tile_set.placed_tiles.end()) return;
    ...
    if (was_autotile)
    {
        for (int dx = -1; dx <= 1; dx++)
        {
            for (int dy = -1; dy <= 1; dy++)
            {
                ...
                if (tile_it == tile_set.placed_tiles.end()) continue;
...
uint16_t ScenePanel::calculate_bitmask(TileSet_Component& tile_set,
                                       const vec2i& pos)
{
    uint16_t mask = 0;
    auto active_set_id = get_set_id(m_active_set);

    if (active_set_id == std::numeric_limits<uint16_t>::max()) return mask;
...
{
    static constexpr uint16_t priority[] = {TL, TR, BL, BR, T, L, R, B};
...
uint16_t ScenePanel::resolve_mask(
    const std::unordered_map<uint16_t, vec2u>& table, uint16_t mask)
{
    while (!table.contains(mask))
    {
        uint16_t reduced = remove_lowest_priority_bit(mask);
        if (reduced == mask) break;
        mask = reduced;
    }
    return mask;
}
...
{
    {
        if (has_selected_entity())
        {
            ...
            {
                bool has_tileset =
                    m_selected_entity.has_component<Tile_Component>() &&
                    m_selected_entity.has_component<TileSet_Component>() &&
                    m_selected_entity.has_component<Texture_Component>();
                bool has_solidset =
                    m_selected_entity.has_component<Tile_Component>() &&
                    m_selected_entity.has_component<SolidSet_Component>();

                if (!has_tileset && !has_solidset) return;
                ...
                if (painting_solid)
                    solid_set =
                        &m_selected_entity.get_component<SolidSet_Component>();
                else
                    tile_set =
                        &m_selected_entity.get_component<TileSet_Component>();
                ...
                if (!m_use_auto_tile && !painting_solid)
                {
                    ...
                    Renderer2D::set_texture(
                        m_selected_entity.get_component<Texture_Component>()
                            .texture);
                    {
                        for (const auto& [position, id] : temp_tiles)
                        {
                            ...
                            trans.position = (position * props.size) +
                                             props.size / 2 + props.offset;
                    ...
                    if (def != tile_set->tile_definitions.end())
                    {
                        ...
                        Transform_Component trans;
                        vec2f current_mouse =
                            EditorLayer::get().get_viewport_mouse_position();

                        vec2i current_tile = {
                            (int)std::floor((current_mouse.x - props.offset.x) /
                                            props.size.x),
                            (int)std::floor((current_mouse.y - props.offset.y) /
                                            props.size.y)};

                        trans.position = (current_tile * props.size) +
                                         props.size / 2 + props.offset;
...
void ScenePanel::update_neighbour(TileSet_Component& tile_set,
                                  const vec2i& start)
{
    ...
    for (int dx = -1; dx <= 1; dx++)
        for (int dy = -1; dy <= 1; dy++)
            if (dx || dy) q.push({start.x + dx, start.y + dy});
    ...
    while (!q.empty())
    {
        ...
        if (self_it == tile_set.placed_tiles.end()) continue;

        Tile& self = self_it->second;
        if (self.set_id != set_id || !self.use_autotile) continue;
        ...
        if (id_it != table.end() && self.tile_id != id_it->second)
        {
            ...
            for (int dx = -1; dx <= 1; dx++)
                for (int dy = -1; dy <= 1; dy++)
                    if (dx || dy) q.push({pos.x + dx, pos.y + dy});
        }
    }
}
}  // namespace ag
