...
void Sandbox2D::on_event(Event& event)
{
    ...
    dispatcher.Dispatch<MouseButtonPressedEvent>(
        AERO_BIND_EVENT_FN(Sandbox2D::on_mouse_pressed));
    dispatcher.Dispatch<WindowResizeEvent>(
        AERO_BIND_EVENT_FN(Sandbox2D::on_window_resize));
    dispatcher.Dispatch<WindowCloseEvent>(
        AERO_BIND_EVENT_FN(Sandbox2D::on_window_close));
}
...
void Sandbox2D::entity_selection()
{
    ...
    if (mouse_position.x >= 0 && mouse_position.y >= 0)
    {
        ...
        if (pixel_data >= 0)
        {
            ...
            {
                auto& props =
                    m_hover_entity.get_component<ButtonState_Component>();
...
bool Sandbox2D::on_mouse_pressed(MouseButtonPressedEvent& e)
{
    ...
    {
        if (!m_hover_entity) return false;
...
void Sandbox2D::load_project_data()
{
    ...
    AssetManager::init(pak_path, fallback_base);

    json j;
    if (AssetManager::is_packed())
    {
        j = AssetManager::read_json("settings.json");
    }
    else
    {
        std::string setting_path =
            get_appdata_path() + "/AEROEngine/settings.json";
        Helper::normalize_path(setting_path);
        j = AssetManager::read_json(setting_path);
    }
    if (j.is_null())
    {
    ...
    if (AssetManager::is_packed())
    {
        ...
        json proj_json = AssetManager::read_json(proj_file_path);
