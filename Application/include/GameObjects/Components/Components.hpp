...
struct Base_Component
{
    ...
    {
        if (!entity.has_component<T>()) entity.add_component<T>();
    }
    static void remove_component(Entity entity)
    {
        if (entity.has_component<T>()) entity.remove_component<T>();
    }
...
struct Transform_Component
{
    ...
    static void get_local_transform(Entity entity,
...
class GroundContactListener : public b2ContactListener
{
   public:
    ...
    std::unordered_map<std::string, std::unordered_set<AG_uint>>
        sensor_collisions;
    ...
    void BeginContact(b2Contact* contact) override
    {
        ...
        if (fixtureA->IsSensor() && fixtureA->GetUserData().pointer != 1)
        {
            auto* shapeA = reinterpret_cast<CollisionShape_Data*>(
                fixtureA->GetUserData().pointer);
            if (shapeA)
            {
        ...
        {
            auto* shapeB = reinterpret_cast<CollisionShape_Data*>(
    ...
    void EndContact(b2Contact* contact) override
    {
        ...
        if (collisions[entity1].empty()) collisions.erase(entity1);
        if (collisions[entity2].empty()) collisions.erase(entity2);

        // Handle foot sensors
        if (fixtureA->IsSensor() && fixtureA->GetUserData().pointer == 1)
        {
            ground_contacts[entity1]--;
            if (ground_contacts[entity1] <= 0) ground_contacts.erase(entity1);
        }
        else if (fixtureB->IsSensor() && fixtureB->GetUserData().pointer == 1)
        {
            ground_contacts[entity2]--;
            if (ground_contacts[entity2] <= 0) ground_contacts.erase(entity2);
        }
    ...
    bool is_collided(AG_uint entity_id_1, AG_uint entity_id_2) const
    {
        ...
        if (it == collisions.end()) return false;
...
struct Text
{
    ...
    static vec2f calc_line_text_size(const Text& text, const vec2f& s,
                                     const vec2f& longest_size,
                                     size_t start_index, size_t* break_index);
    static vec2f center_text(const Text& text,
                             const Transform_Component& transform,
                             const vec2f& longest_size);
    static vec2f center_single_line_text(const Text& text,
                                         const Transform_Component& transform,
                                         const vec2f& longest_size,
                                         size_t start_index,
                                         size_t* break_index);
...
{
    vec2u size = {100, 20};
    Text_Allignment_Horizontal h_allignment =
        Text_Allignment_Horizontal::Center;
    Text_Allignment_Vertical v_allignment = Text_Allignment_Vertical::Center;
    bool uniform = true;
