...
MemberType safe_get_comp_value(Entity& entity, MemberType Component::* member,
...
void ScriptBinding::register_keyboard_polling()
{
    ...
    lua.new_enum<KeyCode>("Key", {// Printable keys
                                  {"Space", Key::Space},
    ...
    keyboard.set_function("is_key_down",
                          [](KeyCode key) -> bool { return Keyboard::is_key_pressed(key); });

    keyboard.set_function("is_key_up",
...
void ScriptBinding::register_mouse_polling()
{
    ...
    lua.new_enum<MouseCode>(
        "Button",
        {{"Left", Button::Button0}, {"Right", Button::Button1}, {"Middle", Button::Button2}});

    // Mouse button state functions
    mouse.set_function("is_button_down",
                       [](MouseCode button) -> bool { return Mouse::is_mouse_pressed(button); });

    mouse.set_function("is_button_up",
                       [](MouseCode button) -> bool { return Mouse::is_mouse_released(button); });
    ...
    mouse.set_function("set_mouse_position",
...
void ScriptBinding::register_events()
{
    ...
    lua.new_enum<Event_Type>("EventType", {{"Key_Pressed", Event_Type::Key_Pressed},
    ...
    lua.new_usertype<Event>("Event", "get_type", &Event::get_event_type);

    // KEYBOARD EVENT FUNCTIONS

    lua.set_function("get_key",
                         ...
                         switch (type)
                         {
                             case Event_Type::Key_Pressed:
                                 return sol::make_object(
                                     lua, static_cast<KeyPressedEvent&>(event).get_key_code());
                             case Event_Type::Key_Released:
                                 return sol::make_object(
                                     lua, static_cast<KeyReleasedEvent&>(event).get_key_code());
                             case Event_Type::Key_Typed:
                                 return sol::make_object(
                                     lua, static_cast<KeyTypedEvent&>(event).get_key_code());
                             default:
                                 return sol::nil;
                         }
                     });
    ...
    lua.set_function("is_key_event",
                     [](Event& event) -> bool
                     {
                         ...
                         return type == Event_Type::Key_Pressed ||
                                type == Event_Type::Key_Released || type == Event_Type::Key_Typed;
                     });
        ...
        [&lua](Event& event) -> sol::object
        {
            ...
            switch (type)
            {
                case Event_Type::Mouse_Button_Pressed:
                    return sol::make_object(
                        lua, static_cast<MouseButtonPressedEvent&>(event).get_mouse_button());
                case Event_Type::Mouse_Button_Released:
                    return sol::make_object(
                        lua, static_cast<MouseButtonReleasedEvent&>(event).get_mouse_button());
                default:
                    return sol::nil;
            }
        });
    ...
    lua.set_function("get_scroll",
                     [&lua](Event& event) -> sol::object
                     {
                         if (event.get_event_type() == Event_Type::Mouse_Scrolled)
                         {
                             ...
                             return sol::make_object(
                                 lua, std::make_pair(e.get_offsetX(), e.get_offsetY()));
                         }
                         return sol::nil;
                     });

    lua.set_function("get_scroll_x",
                     [&lua](Event& event) -> sol::object
                     {
                         if (event.get_event_type() == Event_Type::Mouse_Scrolled)
                         {
                             return sol::make_object(
                                 lua, static_cast<MouseScrolledEvent&>(event).get_offsetX());
                         }
                         return sol::nil;
                     });

    lua.set_function("get_scroll_y",
                     [&lua](Event& event) -> sol::object
                     {
                         if (event.get_event_type() == Event_Type::Mouse_Scrolled)
                         {
                             return sol::make_object(
                                 lua, static_cast<MouseScrolledEvent&>(event).get_offsetY());
                         }
                         return sol::nil;
                     });
...
void ScriptBinding::register_color()
{
    ...
    lua.new_usertype<Color>(
        ...
        sol::meta_function::equal_to, &Color::operator==, sol::meta_function::to_string,
        [](const Color& c)
        {
...
void ScriptBinding::register_vec2()
{
    ...
    lua.new_usertype<vec2f>(
        "vec2f", sol::call_constructor,
        sol::factories([](float x, float y) { return vec2f(x, y); }), "x", &vec2f::x, "y",
        &vec2f::y,
        ...
        sol::resolve<vec2f(const vec2f&) const>(&vec2f::operator*), sol::meta_function::division,
        sol::resolve<vec2f(const vec2f&) const>(&vec2f::operator/), sol::meta_function::equal_to,
        sol::resolve<bool(const vec2f&) const>(&vec2f::operator==), sol::meta_function::to_string,
        [](const vec2f& v)
        { return "vec2f(" + std::to_string(v.x) + ", " + std::to_string(v.y) + ")"; },

        "length", [](const vec2f& v) { return std::sqrt(v.x * v.x + v.y * v.y); }, "normalized",
        [](const vec2f& v)
        {
            float len = std::sqrt(v.x * v.x + v.y * v.y);
            return len > 0 ? vec2f(v.x / len, v.y / len) : vec2f(0, 0);
        },
        "dot", [](const vec2f& v1, const vec2f& v2) { return v1.x * v2.x + v1.y * v2.y; });

    // vec2i (integer vector)
    lua.new_usertype<vec2i>(
        "vec2i", sol::call_constructor, sol::factories([](int x, int y) { return vec2i(x, y); }),
        "x", &vec2i::x, "y", &vec2i::y,
        ...
        sol::meta_function::multiplication,
        sol::resolve<vec2i(const vec2i&) const>(&vec2i::operator*), sol::meta_function::division,
        sol::resolve<vec2i(const vec2i&) const>(&vec2i::operator/), sol::meta_function::equal_to,
        sol::resolve<bool(const vec2i&) const>(&vec2i::operator==), sol::meta_function::to_string,
        [](const vec2i& v)
        { return "vec2i(" + std::to_string(v.x) + ", " + std::to_string(v.y) + ")"; });

    // vec2u (unsigned integer vector)
    lua.new_usertype<vec2u>(
        "vec2u", sol::call_constructor,
        sol::factories([](AG_uint x, AG_uint y) { return vec2u(x, y); }), "x", &vec2u::x, "y",
        &vec2u::y,
        ...
        sol::meta_function::multiplication,
        sol::resolve<vec2u(const vec2u&) const>(&vec2u::operator*), sol::meta_function::division,
        sol::resolve<vec2u(const vec2u&) const>(&vec2u::operator/), sol::meta_function::equal_to,
        sol::resolve<bool(const vec2u&) const>(&vec2u::operator==), sol::meta_function::to_string,
        [](const vec2u& v)
...
void ScriptBinding::register_node()
{
    ...
    lua.set_function(
        ...
        {
            auto rotation =
                detail::safe_get_comp_value(entity, &Transform_Component::rotation, 0.0f);
            rotation += delta;
            detail::safe_set_comp_value(entity, &Transform_Component::rotation, rotation);
        });
