...
void ScriptManager::init()
{
  ...
  s_lua_state.open_libraries(sol::lib::base,
                             sol::lib::math,
                             sol::lib::table,
                             sol::lib::string,
                             sol::lib::os);

  s_lua_state.set_function("aero_print",
                           [](const std::string& msg) { AERO_CORE_INFO("[Lua] {0}", msg); });

#ifdef AERO_SERVER
  RegisterFunction::server_init();
#else
  RegisterFunction::client_init();
#endif
