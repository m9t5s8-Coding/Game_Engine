...
bool OpenALAudioBuffer::load_mp3(const std::string& path)
{
  ...
  alBufferData(m_ID,
               out_format,
               out_data.data(),
               static_cast<ALsizei>(out_data.size() * sizeof(short)),
               out_sample_rate);
...
bool OpenALAudioBuffer::load_mp3_from_memory(const uint8_t* data, size_t size)
{
  ...
  alGenBuffers(1, &m_ID);
  alBufferData(m_ID, format, pcm.data(), (ALsizei)(pcm.size() * sizeof(int16_t)), mp3.sampleRate);
...
bool OpenALAudioBuffer::load_wav(const std::string& path)
{
  ...
  alGenBuffers(1, &m_ID);
  alBufferData(m_ID, format, pcmData.data(), pcmData.size() * sizeof(int16_t), wav.sampleRate);
  drwav_uninit(&wav);
...
bool OpenALAudioBuffer::load_wav_from_memory(const uint8_t* data, size_t size)
{
  ...
  alGenBuffers(1, &m_ID);
  alBufferData(m_ID, format, pcm.data(), (ALsizei)(pcm.size() * sizeof(int16_t)), wav.sampleRate);
...
std::vector<uint8_t> OpenALAudioBuffer::read_audio_bytes(const std::string& path)
{
  if (AssetManager::is_packed(AssetManager::Domain::Project))
    return AssetManager::read_bytes(path, AssetManager::Domain::Project);
