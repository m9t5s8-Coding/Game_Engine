...
void WindowsWindow::init(const WindowProps& props)
{
  ...
  m_Window = glfwCreateWindow(static_cast<int>(props.Size.x),
                              static_cast<int>(props.Size.y),
                              props.Title.c_str(),
                              nullptr,
                              nullptr);
  ...
  glfwSetKeyCallback(
      m_Window,
      [](GLFWwindow* window, const int key, const int scancode, const int action, const int mode)
      {
...
void WindowsWindow::set_full_screen()
{
  ...
  #ifdef PLATFORM_LINUX
    glfwSetWindowMonitor(m_Window, nullptr, 0, 0, m_win_data.old_size.x, m_win_data.old_size.y, 0);
  #else
    glfwSetWindowMonitor(m_Window,
                         nullptr,
                         m_win_data.old_pos.x,
                         m_win_data.old_pos.y,
                         m_win_data.old_size.x,
                         m_win_data.old_size.y,
                         0);
  #endif
