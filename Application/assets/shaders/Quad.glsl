...
void main()
{
    ...
    else if(a_type == 2)
    {
        ...
        float u = a_flip.x > 0.0 ? mix(a_texture_rect.x, a_texture_rect.x + a_texture_rect.z, a_pos.x)
                                 : mix(a_texture_rect.x + a_texture_rect.z, a_texture_rect.x, a_pos.x);
        float v = a_flip.y > 0.0 ? mix(a_texture_rect.y, a_texture_rect.y + a_texture_rect.w, a_pos.y)
    ...
    else if(a_type == 3)
    {
        ...
        frag_pos = vec2(0.0); // not needed for text
        size = a_size;
...
void main()
{
    if(v_type == 0)
    {
        ...
        if(is_corner)
        {
            vec2 corner_center = vec2(frag_pos.x < total_radius ? total_radius : size.x - total_radius,
    ...
    else if(v_type == 1)
    {
        ...
        float ellipse_dist = (delta.x*delta.x)/(radius_x*radius_x) + (delta.y*delta.y)/(radius_y*radius_y);
        if(ellipse_dist > 1.0)
            discard;
    ...
    {
        // Text with SDF
        vec3 col = texture(u_textures[v_slot], tex_coord).rgb;
